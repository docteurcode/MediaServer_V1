...
from movie.models import Mojaloss, NeedToDownload
    ...
    for each_movie_url in movies_url:
        ...
        try:
            ...
            if(len(movie_full_title) > 1):
                ...
                if (movie_download_links.split(".")[-1] == 'mp4'):
                    manually_download = NeedToDownload(
                        title=movie_title, year=movie_year, url=movie_download_links)
                    manually_download.save()
                else:
